...
case class MyList(es:Seq[Exp]) extends Exp            // [ e1, e2, ... ]
case class Head(e:Exp) extends Exp                    // e.head
case class Tail(e:Exp) extends Exp                    // e.tail
...
object ParseL extends StandardTokenParsers with PackratParsers {
  ...
  // list
  lazy val listP: P[MyList] = "[" ~> repsep(ExpP, ",") <~ "]" ^^ (MyList)

  // head
  lazy val headP: P[Head] = E <~ "." ~ "head" ^^ (Head)

  // tail
  lazy val tailP: P[Tail] = E <~ "." ~ "tail" ^^ (Tail) 
}
